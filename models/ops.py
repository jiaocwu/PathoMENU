...
def centralize(
    ...
    entities_centroid = scatter(batch[key], batch_index, dim=0, reduce="mean")
...
class PolynomialCutoff(nn.Module):
    ...
    def polynomial_cutoff(
        ...
        if not p >= 2.0:
            error_msg = f"Exponent p={p} has to be >= 2."
...
class TensorLayerNorm(nn.Module):
    def __init__(self, hidden_channels, trainable, **kwargs):
...
class NodeInit(MessagePassing):
    def __init__(self, hidden_channels, num_rbf, cutoff, max_z=100, activation=F.silu, proj_ln='',
        ...
        self.concat = concat
        self.fc1_stru = nn.Linear(1280, 512)
    ...
    def forward(self, z, x, edge_index, edge_weight, edge_attr):
        ...
        x_neighbors = self.fc2_stru(self.silu(self.fc1_stru(z)))
        if not self.concat:
