...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--mutant-pdb-dir", required=True, type=Path)
    parser.add_argument(
        "--wild-type-pdb-dir",
        required=True,
        type=Path,
        help="Original PDB directory used as the authoritative source of metal ions.",
    )
...
def main() -> None:
    ...
    required = {"sample_id", "pdb_id", "pdb_position", "chain", "metal ion", "mut_aa"}
    ...
    for index, row in dataframe.iterrows():
        ...
        source = args.mutant_pdb_dir / f"{sample_id}.pdb"
        metal_source = args.wild_type_pdb_dir / f"{row['pdb_id'].upper()}.pdb"
        ...
        try:
            residue_count, metal_count = prepare_local_structure(
                ...
                metal_source_path=metal_source,
                output_path=destination,
                chain_id=row["chain"],
                pdb_position=row["pdb_position"],
                expected_amino_acid=row["mut_aa"],
    ...
    if failures:
        raise RuntimeError("Failed to prepare mutant local structures:\n" + "\n".join(failures))
